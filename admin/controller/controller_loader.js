/**
 * Loader for admin controller
 * 
 * Configuration for controllers is in admin_controller_config.js file.
 */

var controller_loader = {};

var admin_controller_config = require('../config/admin_controller_config');
...
// function for processing controller configuration in admin_controller_config.js file
function initRoutes(app, router, upload) {
    // load admin controllers
	var infoLen = admin_controller_config.length;
	logger.debug('The number of controllers in admin_controller_config : %d', infoLen);
  
	for (var i = 0; i < infoLen; i++) {
		var curItem = admin_controller_config[i];
    ...
    // load controllers
	var infoLen2 = controller_config.length;
	logger.debug('The number of controllers in controller_config : %d', infoLen2);
  
	for (var i = 0; i < infoLen2; i++) {
		var curItem = controller_config[i];
			
		// Check the file exists or not
        var filename = path.join(__dirname, '../../controller', curItem.file);
        //logger.debug('filename #' + i + ' : ' + filename);
