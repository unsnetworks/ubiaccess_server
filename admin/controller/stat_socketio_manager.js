 ...
 * search
 *
 * GET http://host:port/manager/statsocketio?criteria
 */
 
var searchStatSocketIO = function(req, res) {
	logger.debug('searchStatSocketIO 호출됨.');
    logger.debug('요청 패스 -> /manager/statsocketio?criteria GET');
	...
	try {
        ...
        if (database_mysql.pool) {
            ...
            database_mysql.statsocketio.search(database_mysql.pool, paramColumnNames, page, perPage, data, function(err, rows) {
                // 에러 발생 시 - 클라이언트로 에러 전송
                if (err) {
                    sendJson(res, paramRequestCode, 400, 'statsocketio search 중 에러 발생', 'error', err);
                ...
                if (rows) {
                    ...
                    database_mysql.statsocketio.count(database_mysql.pool, function(err2, countRows) {
                        // 에러 발생 시 - 클라이언트로 에러 전송
                        if (err2) {
                            sendJson(res, paramRequestCode, 400, 'statsocketio count 중 에러 발생', 'error', err2);
                        ...
                        sendJsonResult(res, paramRequestCode, 200, 'statsocketio search 성공', 'userlist', rows, page, perPage, countRows[0].count);
                         
                    });
                } else {
                    sendJson(res, paramRequestCode, 400, 'statsocketio search 실패', 'string', 'statsocketio search 결과가 없습니다.');
                }
            });
...
module.exports.searchStatSocketIO = searchStatSocketIO;
