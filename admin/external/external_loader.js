...
function load(app, config) {
	...
	for (var i = 0; i < external_config.length; i++) {
        ...
        if (fs.existsSync(filename + '.js')) {
            ...
            if (curItem.active) {
                if (!config.external[curItem.external_index].count) {
                    config.external[curItem.external_index].count = 0;
                }
                
                curModule.init(app, config, config.external[curItem.external_index], external_loader.external, curItem.external_index, config.external[curItem.external_index].count);
                
                
                logger.debug('external_index -> ' + String(curItem.external_index));
                 
               
               if (!external_loader.external.listeners[String(curItem.external_index)].socketListener) {
                   external_loader.external.listeners[String(curItem.external_index)].socketListener = [];
                   logger.debug('socketListener is set to array.');
               } 
                external_loader.external.listeners[String(curItem.external_index)].socketListener.push(curModule);
                logger.debug('Listener object is added to socketListener.');

                logger.debug('DEBUG 1 -> ' + JSON.stringify(external_loader.external.listeners[String(curItem.external_index)].socketListener));
                
                
                config.external[curItem.external_index].count += 1;
            }
