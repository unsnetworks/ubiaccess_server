...
function tableinitTable(tableTag, colNames, colAliases, columnDefs, aoColumns, sModifyButton, sDeleteButton, baseUrl, type, listPath, addPath, modifyPath, deletePath, icon) {
    ...
    if (tableTag == '#table1') {
        DBDataTable = $(tableTag).dataTable({
            "bInfo": false,
            "bLengthChange": false,
            "searching": false,
            "bPaginate": false,
        ...
        RouteDataTable = $(tableTag).dataTable({
            "bInfo": false,
            "bLengthChange": false,
            "searching": false,
            "bPaginate": false,
        ...
        SocketDataTable = $(tableTag).dataTable({
            "bInfo": false,
            "bLengthChange": false,
            "searching": false,
            "bPaginate": false,
 ...
 */
function DBrequestList(page, perPage) {

    console.log('DBrequestList 호출됨.');
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log('DBrequestList에 대한 응답을 받았음.');
            console.log(data);

            var dataObj = JSON.parse(data);
            if (dataObj.code == 200) {
                // set table data
                DBsetResultToTable(dataObj.result, columnNames, '#table1', dataObj.page, dataObj.perPage, dataObj.totalRecords);
...
function routerequestList(page, perPage) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (dataObj.code == 200) {
                // set table data
                RouteSetResultToTable(dataObj.result, columnNames, '#table2', dataObj.page, dataObj.perPage, dataObj.totalRecords);

                // set page buttons using pagination.js
                setPagination(dataObj.page, dataObj.perPage, dataObj.totalRecords, '#pageButtons1');
            } else {
                showErrorDialog(dataObj.code, dataObj.message);
            }
...
function socketrequestList(page, perPage) {
    console.log('requestList 호출됨.');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (dataObj.code == 200) {
                // set table data
                socketioSetResultToTable(dataObj.result, columnNames, '#table3', dataObj.page, dataObj.perPage, dataObj.totalRecords);

                // set page buttons using pagination.js
                setPagination(dataObj.page, dataObj.perPage, dataObj.totalRecords, '#pageButtons2');
            } else {
                showErrorDialog(dataObj.code, dataObj.message);
            }
...
function DBsetResultToTable(result, attrNames, dataTableTag, page, perPage, totalRecords) {
    console.dir(result);
    ...
    result.forEach(function (elem, index) {
        ...
        /*onsole.log('attrValues' + attrValues);
        console.log('row' + row);*/
        ...
        if (showModifyButton) {
            ...
            buttonHtml += "<button type='button' class='edit-modal btn btn-success btn-xs' onclick='doGetFile(\"" + elem['file'] + "\")'><span class='glyphicon glyphicon-list-alt'></span>소스</button>";
...
function RouteSetResultToTable(result, attrNames, dataTableTag, page, perPage, totalRecords) {
    console.dir(result);
    ...
    result.forEach(function (elem, index) {
        ...
        if (showModifyButton) {
            // button size : btn-sm btn-xs
            buttonHtml += "<button type='button' class='edit-modal btn btn-success btn-xs' onclick='routedoGetFile(\"" + elem['file'] + "\")'><span class='glyphicon glyphicon-list-alt'></span>소스</button>";
...
function socketioSetResultToTable(result, attrNames, dataTableTag, page, perPage, totalRecords) {
    console.dir(result);
    console.log('socketioSetResultToTable 호출');
    ...
    result.forEach(function (elem, index) {
        ...
        if (showModifyButton) {
            // button size : btn-sm btn-xs
            buttonHtml += "<button type='button' class='edit-modal btn btn-success btn-xs' onclick='socketIodoGetFile(\"" + elem['file'] + "\")'><span class='glyphicon glyphicon-list-alt'></span>소스</button>";
...
function setPagination(page, perPage, totalRecords, pageButtonsTag) {
    ...
    console.log('pageButtonsTag');
    console.log(pageButtonsTag);

    var path;
    if (pageButtonsTag == '#pageButtons') {
        path = 'DBrequestList';
    } else if (pageButtonsTag == '#pageButtons1') {
        path = 'routerequestList';
    } else if (pageButtonsTag == '#pageButtons2') {
        path = 'socketrequestList';
    }
    ...
    if (currentMin > 1) {
        // First Page
        buttonHtml += "  <li>";
        buttonHtml += "    <a href='javascript:" + path + "(1, " + perPage + ")'>";
        buttonHtml += "      <span class='glyphicon glyphicon-step-backward' aria-hidden='true'></span>";
        buttonHtml += "    </a>";
        buttonHtml += "  </li>";
        // Previous Page
        buttonHtml += "  <li>";
        buttonHtml += "    <a href='javascript:" + path + "(" + previousPage + ", " + perPage + ")'>";
    ...
    for (var i = initial; i < max; i++) {
        if (i != page) {
            buttonHtml += "  <li><a href='javascript:" + path + "(" + i + ", " + perPage + ")'>";
            buttonHtml += "" + i;
            buttonHtml += "</a></li>";
        } else {
    ...
    if (currentMax < pageCount) {
        // Next Page
        buttonHtml += "  <li>";
        buttonHtml += "    <a href='javascript:" + path + "(" + nextPage + ", " + perPage + ")'>";
        buttonHtml += "      <span class='glyphicon glyphicon-chevron-right' aria-hidden='true'></span>";
        buttonHtml += "    </a>";
        buttonHtml += "  </li>";
        // Last Page
        buttonHtml += "  <li>";
        buttonHtml += "    <a href='javascript:" + path + "(" + (pageCount) + ", " + perPage + ")'>";
