...
function initTable(tableTag, colNames, colAliases, columnDefs, aoColumns, sModifyButton, sDeleteButton, baseUrl, type, listPath, addPath, modifyPath, deletePath, icon) {
    ...
    $(tableTag + ' tbody').on( 'click', 'td', function () {
        console.dir( table1 );
    });
    ...
    result.forEach(function(elem, index) {
        ...
        for (var i = 0; i < attrNames.length; i++) {
            if (i > 0) {
                if (i == 6) {
                    // date format
                    var curTimeObj = moment(elem[attrNames[i]], 'YYYY-MM-DDTHH:mm:ss.000Z');
                    var curTime = curTimeObj.format('YYYY-MM-DD HH:mm:ss');
                    row.push(curTime);
                } else {
        ...
        if (showModifyButton) {
            ...
            buttonHtml += "<button type='button' class='edit-modal btn btn-success btn-xs' onclick='doShowData(\"" + index + "\")'><span class='glyphicon glyphicon-list-alt'></span>보기</button>";
        }
